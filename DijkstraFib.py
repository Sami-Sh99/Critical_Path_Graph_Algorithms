...
def dijkstraFib(graph: 'networkx.classes.graph.Graph', start: str, end: str) -> 'List':
    ...
    while heap.total_nodes:
        ...
        for neighbor in dict(graph.adjacency()).get(current):
            cost=graph.get_edge_data(current,neighbor).get('weight')
    ...
    return backtrace(prev, start, end), distance
